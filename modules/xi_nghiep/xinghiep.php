...
if (isset($_GET['search_xn'])) {
    ...
    $query = "
    SELECT enterprise.*, 
           (SELECT COUNT(DISTINCT jporder.mdh) 
            FROM jporder 
            WHERE jporder.xi_nghiep = enterprise.xi_nghiep) AS total_orders,
           (SELECT COUNT(DISTINCT student.mhv) 
            FROM student 
            LEFT JOIN jporder ON student.mdh = jporder.mdh 
            WHERE jporder.xi_nghiep = enterprise.xi_nghiep) AS total_students
    FROM enterprise
    WHERE (mdn LIKE '%".$key."%' 
    OR ten_giam_doc LIKE '%".$key."%') 
    ORDER BY SUBSTRING(enterprise.mdn, -3) ASC 
                ...
                <table class="table table-hover table-stripe">
                    ...
                    while ($row = mysqli_fetch_assoc($res)) {
                        ...
                        <tr id="box<?php echo $row['mdn']?>">
                            ...
                            <td style="white-space: nowrap;"><a href="index.php?action=view&mxn=<?php echo $row['mdn'] ?>" class="view" style="text-decoration: none;color:black"><?php echo $row['mdn'] ?></a></td>
                            ...
                            <td style="white-space: nowrap;"><?php echo $row['total_orders'] ?></td>
                            <td style="white-space: nowrap;"><?php echo $row['total_students'] ?></td>
