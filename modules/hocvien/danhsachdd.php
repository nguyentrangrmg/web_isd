...
while ($row = mysqli_fetch_assoc($xiNghiepResult)) {
    ...
    <div class="loai_hv">
        <ul class="nav nav-tabs">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="?type=dd" style="text-decoration: none;color:black">Kỹ năng đặc
                    định</a>
            </li>
        </ul>
    </div>
    <div class="function" style="text-align: right;">
        <div style="display: inline-block;">
            ...
            <a href="modules/hocvien/filter-hv.php"><button class="ec-ec" id="filterButton">Bộ lọc</button>
        </div>

        <form action="" method="GET" style="display: inline-block;">
            <input type="text" class="search-input" placeholder="Search..." name="key_search"
                value="<?php echo isset($key) ? $key : ''; ?>">
            <input type="hidden" name="type" value="dd">
            <button type="submit" name="search_hv" class="search-button"><i
        ...
        <div class="popup-content">
            ...
            <div>
                <div class="checkbox-item" id="lichSuXuatKhauLaoDong">
                    ...
                    <label for="lichSuXuatKhauLaoDong">Lịch sử xuất khẩu lao động</label>
                </div>
                <div class="checkbox-item">
            ...
            <form method="post" id="frm">
                <table class="table table-hover table-stripe">
                    ...
                    <tr id="box<?php echo $row['mhv']?>">
                        <td><input type="checkbox" id="<?php echo $row['mhv']?>" name="checkbox[]"
                                value="<?php echo $row['mhv']?>" /></td>
                        ...
                        <td style="white-space: nowrap;"><?php echo date('d/m/Y', strtotime($row['ngay_nhaphoc'])) ?>
                        </td>
                        ...
                        <td>
                            <div style="display:flex" class="action-buttons">
                                <form action="#" method="#">
                                    <!-- bỏ cái này thì nút edit dòng đầu tiên sẽ không hoạt động -->
                                    <button type="hidden"
                                        style="padding:0px;border: none; background: none; color: inherit;"></button>
                                </form>
                                <a href="index.php?action=view&mhv=<?php echo $row['mhv']; ?>" class="view"
                                ...
                                <form action="?edit=sua&mhv=<?php echo $row['mhv']; ?>" method="GET">
                                    ...
                                    <button class="sua" style="border: none; background: none; color: inherit;"><i
                                            class="fas fa-pen edit"></i></button>
                                </form>
                                <form action="modules/hocvien/delete.php" method="POST">
                                    <input type="hidden" name="delete" value="<?php echo $row['mhv']; ?>">
                                    <button type="submit" class="xoa"
                                        style="border: none; background: none; color: inherit;"><i
                                            class="fas fa-trash trash"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
                ...
                if ($pages > 0) { ?>
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination d-flex justify-content-center">
                            <li class="page-item">
                                <a class="page-link"
                                    href="?pageo=<?php echo max(1, $current_page - 1); ?>&type=<?php echo $type; ?>"
                                    aria-label="Previous">
                        ...
                        if ($start_page > 1) {
                            ?>
                            <li class="page-item"><a class="page-link" href="?pageo=1&type=<?php echo $type; ?>">1</a>
                            </li>
                            <?php
                            if ($start_page > 2) {
                        ...
                        for ($i = $start_page; $i <= $end_page; $i++) {  
                            ...
                            <li class="page-item"><a class="page-link"
                                    href="?pageo=<?php echo $i; ?>&type=<?php echo $type; ?>"><?php echo $i; ?></a></li>
                            <?php
                            } else { ?>
                            <li class="current-page page-item"><a style="color:white;background:#374375;"
                                    class="page-link"
                                    href="?pageo=<?php echo $i; ?>&type=<?php echo $type; ?>"><?php echo $i; ?></a></li>
                            <?php 
                            }
                        ...
                        if ($end_page < $pages) {
                            ?>
                            <li class="page-item"><a class="page-link"
                                    href="?pageo=<?php echo $pages; ?>&type=<?php echo $type; ?>"><?php echo $pages; ?></a>
                            </li>
                            <?php
                        }
                        ?>
                            <li class="page-item">
                                <a class="page-link"
                                    href="?pageo=<?php echo min($pages, $current_page + 1); ?>&type=<?php echo $type; ?>"
                                    aria-label="Next">
    ...
    function delete_all() {
        if (jQuery('input[type=checkbox]:checked').length > 0) {
            var check = confirm(
                "Bạn có muốn chuyển học viên này vào thùng rác? (Dữ liệu của học viên sẽ bị xóa sau 30 ngày)");
            if (check == true) {
                jQuery.ajax({
